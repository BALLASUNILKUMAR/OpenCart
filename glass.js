...
document.addEventListener('DOMContentLoaded', function() {
    const checkoutBtn = document.getElementById('proceedToBuy');
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', function() {
            console.log("Proceed to Buy clicked ✅");

            fetch('glass.php?action=checkLogin')
                .then(res => res.json())
                .then(data => {
                    ...
                    } else if (!data.addressComplete) {
                        // Address missing → dashboard
                        window.location.href = 'dashboard.php?update_address=1';
                    } else {
                        // Logged in + address OK → open modal
                        showCheckoutModal(data.user, cart);
                    }
                })
                .catch(err => {
    ...
    // Confirm purchase → save cart then redirect
    const confirmBtn = document.getElementById('confirmPurchase');
    if (confirmBtn) {
        confirmBtn.addEventListener('click', function() {
            fetch('glass.php?action=saveCart', {
                method: 'POST',
...
function showCheckoutModal(user) {
    ...
    let addressHtml = `
        ...
        <p><strong>Flat/House No:</strong> ${user.house_no}, ${user.street}</p>
    ...
    let cartHtml = `
        <p style="border-top:1px solid #ddd; margin-top:10px; padding-top:10px; font-size:16px;">
            £${total.toFixed(2)}
        </p>
    `;

    document.getElementById('checkoutAddress').innerHTML = addressHtml;
    document.getElementById('checkoutProducts').innerHTML = cartHtml;
